...
class SimpleSampler // Class to sample a LiDAR point cloud at an (x, y) position
	{
	/* Elements: */
	private:
	double pos[2]; // Sample point's (x, y) position
	double gridCellSize[2]; // Cell size of target grid in x and y
	double scale; // Scale factor to condition the least-squares matrix
	#if 1
	double sum;
	double weight;
	#else
	Geometry::Matrix<double,3,3> ata;
	Geometry::ComponentArray<double,3> atb;
	#endif
	unsigned int numPoints;
	
	/* Constructors and destructors: */
	public:
	SimpleSampler(const double sPos[2],const double sGridCellSize[2])
		#if 1
		:sum(0.0),weight(0.0),
		#else
		:ata(0.0),atb(0.0),
		...
		scale=Math::sqrt(gridCellSize[0]*gridCellSize[1])*0.5;
		}
	...
	void dilate(void)
		...
		weight=0.0;
		#else
		ata=Geometry::Matrix<double,3,3>(0.0);
		atb=Geometry::ComponentArray<double,3>(0.0);
		#endif
		numPoints=0;
		}
	Box calcSampleBox(void) const
		...
		for(int i=0;i<2;++i)
			{
			sampleBox.min[i]=Scalar(pos[i]-gridCellSize[i]);
			sampleBox.max[i]=Scalar(pos[i]+gridCellSize[i]);
			}
	...
	void operator()(const LidarPoint& p)
		{
		#if 1
		double w=Math::max(0.0,(1.0-(p[0]-pos[0])/gridCellSize[0])*(1.0-(p[1]-pos[1])/gridCellSize[1]));
		sum+=p[2]*w;
		weight+=w;
		#else
	...
	double getValue(void) const
		...
		return sum/weight;
		#else
		Geometry::ComponentArray<double,3> x=atb/ata;
		return x[2];
		#endif
		}
	};
...
class LidarGridder:public Vrui::Application,public GLObject
	...
	/* Constructors and destructors: */
	public:
	LidarGridder(int& argc,char**& argv);
...
void LidarGridder::createDem(void)
	...
	Misc::Timer timer;
	std::cout<<"Calculating DEM...   0% done"<<std::flush;
	for(int y=0;y<gridSize[1];++y)
		{
		for(int x=0;x<gridSize[0];++x)
			{
			...
			SimpleSampler sampler(pos,gridCellSize);
			...
			#if 1
			yTotal=sampler.getValue();
			#else
			DemVertex::Normal n(-sampler.getDiff(0),-sampler.getDiff(1),1);
			n.normalize();
			dem(x,y).normal=n;
			#endif
			dem(x,y).position=DemVertex::Position(pos[0],pos[1],yTotal);
			}
		std::cout<<"\rCalculating DEM... "<<std::setw(3)<<(100*(y+1)+gridSize[1]/2)/gridSize[1]<<"% done"<<std::flush;
		}
	std::cout<<std::endl;
...
void LidarGridder::writeBILFile(const char* imageFileName) const
	...
	for(int y=0;y<dem.getSize(1);++y)
		for(int x=0;x<dem.getSize(0);++x)
			imageFile.write(float(dem(x,dem.getSize(1)-1-y).position[2]+lpoOffset[2]));
	}
...
void LidarGridder::writeArcInfoBinaryGridFile(const char* directoryName) const
	...
	if(mkdir(directoryName,0777)<0)
		throw Misc::makeLibcErr(__PRETTY_FUNCTION__,errno,"Cannot create DEM directory %s",directoryName);
	}
...
LidarGridder::LidarGridder(int& argc,char**& argv)
	...
	if(lidarFileName==0)
		throw Misc::makeStdErr(__PRETTY_FUNCTION__,"No LiDAR file name provided");
	
	/* Open the LiDAR data set: */
	lpo=new LidarProcessOctree(lidarFileName,cacheSize*1024*1024);
	lpoDomain=lpo->getDomain();
	for(int i=0;i<3;++i)
		lpoOffset[i]=lpo->getOffset()[i];
	std::cout<<"LiDAR point offset: "<<lpoOffset[0]<<", "<<lpoOffset[1]<<", "<<lpoOffset[2]<<std::endl;
	std::cout<<"LiDAR domain center: "<<lpo->getDomain().getCenter()[0]<<", "<<lpo->getDomain().getCenter()[1]<<", "<<lpo->getDomain().getCenter()[2]<<std::endl;
...
void LidarGridder::resetNavigation(void)
	{
...
VRUI_APPLICATION_RUN(LidarGridder)
